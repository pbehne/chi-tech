...
//###################################################################
/**Solves a groupset using GMRES for POD.*/
void LinearBoltzman::Solver::GMRES_POD(int group_set_num, SweepChunk* sweep_chunk, MainSweepScheduler & sweepScheduler, bool compute_b, bool log_info /*= true*/)
{
  chi_log.Log(LOG_0)
    << "\n\n";
  chi_log.Log(LOG_0)
  ...
  //================================================== Set the action-operator
  if (compute_b)
    MatShellSetOperation(A, MATOP_MULT,(void (*)(void)) NPTMatrixAction_Ax_POD_b);
  else
    MatShellSetOperation(A, MATOP_MULT,(void (*)(void)) NPTMatrixAction_Ax_POD);
  ...
  chi_log.Log(LOG_0) << chi_program_timer.GetTimeString() << " Computing b";
  if (compute_b)
    SetSource(group_set_num,
              true, // Apply material src???
              true);// Suppress MSphi????
  else
    SetSource(group_set_num,
              false, // Apply material src???
              false);// Suppress MSphi????
